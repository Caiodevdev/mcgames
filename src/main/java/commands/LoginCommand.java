...
public class LoginCommand implements CommandExecutor {
    ...
    public LoginCommand(Projeto plugin) {
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        try {
            if (!playerData.isRegistered(player)) {
                MessageUtils.sendTitle(player, "&cErro", "&fVocê precisa se registrar primeiro!", 10, 40, 10);
            ...
            if (playerData.checkPassword(player, args[0])) {
                ...
                MessageUtils.sendTitle(player, "&aLogado!", "&fBem-vindo de volta, " + player.getName() + "!", 10, 60, 10);
